...
from models.model_utils import load_model, make_prediction
...
page = st.sidebar.radio("Go to", ["Home", "Upload Data", "Single Transaction Check", "Model Insights"])
...
if page == "Home":
    ...
    with col2:
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.barplot(x=risk_scores, y=categories, palette='viridis')
    ...
    if uploaded_file is not None:
        try:
            ...
            if model is None or preprocessing is None:
                st.error("Model resources not found. Please run the notebook first to generate the model.")
            else:
                # Process button
...
# Model Insights page
    ...
    with tab3:
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.plot(fpr, tpr, lw=2, label=f'ROC curve (area = 0.96)')
